...
class DominoesGUI(tkinter.Frame):
    def __init__(self, master, rows, cols):
        ...
        ).pack(padx=1, pady=1, anchor=tkinter.W)

        tkinter.Button(menu, text="Reset Game", command=self.reset_click).pack(
    ...
    def perform_best_move(self, limit):
        if not self.game.game_over(self.board.vertical):
            (row, col), best_value, total_leaves = self.game.get_best_move(
                self.board.vertical, limit
            )
            self.board.perform_move(row, col)
